...
async function findDoctor (req, res) {
  ...
  try {
    ...
    if (statusParam) {
      ...
      const findFilteredDoctors = await Doctor.findAll({
        where: { system_status: statusParam.charAt(0).toUpperCase() + statusParam.slice(1).toLowerCase() },
        order: ['id']
      });
    ...
    const allDoctors = await Doctor.findAll({ order: ['id'] });
